...
#ifdef DEBUG_PRINT_CODE
  if (!parser.hadError) {
    disassembleChunk(currentChunk(), function->funcName != NULL
                                         ? function->funcName->chars
                                         : "<script>");
  }
#endif
  assert(current != NULL);
...
/**
 * @brief parse anything >= precedence
 *
 * @param precedence
 */
static void parsePrecedence(const Precedence precedence) {
...
static void initCompiler(Compiler *compiler, FunctionType type) {
  ...
  //* in newFunction, the arity is already set to `0`.
...
static void endScope() {
  ...
  int popn = 0;
  // pop everything in the previous scope.
...
static int emitJump(uint8_t instruction) {
  // by now, we don't know jump distance, so just emit a
  // placeholder.
...
static void patchJump(int offset) {
  ...
  currentChunk()->code[offset] = (jump >> 8) & 0xff; // `jump` second byte
  currentChunk()->code[offset + 1] = jump & 0xff;    // `jump` first byte
}
