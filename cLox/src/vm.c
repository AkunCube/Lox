...
void initVM() {
  ...
  vm.initString = NULL; // For GC safety.
  ...
  for (;;) {
    ...
    switch (instruction = READ_BYTE()) {
      ...
      case OP_GET_PROPERTY: {
        // Check if the top of the stack is an instance.
...
static void runtimeError(const char *format, ...) {
  ...
  /* The interpreter advances past each instruction
     before executing it. So we need minus 1. */
  assert(vm.frameCount > 0);
...
/**
 * @brief nil and false are falsey.
...
static bool call(ObjClosure *closure, int argCount) {
  ObjFunction *prototype = closure->function;

  if (argCount != prototype->arity) {
    runtimeError("Expected %d arguments but got %d.", prototype->arity,
                 argCount);
    return false;
  }
